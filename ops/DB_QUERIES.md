# DB Query Snippets — Tina Diet Production

Curated SQLite queries for ops + monitoring. Designed for **Railway Console**
(backend service → Console tab) since the SQLite db lives on Railway volume at
`/data/app.db`. No `sqlite3` CLI in container — use `node -e` inline.

> **Quoting convention**: outer single quotes (bash escape-free), inner JS
> double quotes. For multi-line SQL with `'` chars inside, escape with `\"`.

> **Safety**: read-only queries are zero risk. Mutation queries (grant/revoke
> premium, delete user) are clearly marked `⚠ MUTATION` — double-check user_id
> before running.

---

## 0. Quick health snapshot
...
### 1.2 List premium users (with days remaining)
...
### 1.3 List users who signed up but never completed profile (drop-off)
...
### 1.4 List active users (logged food in last 7 days)
...
### 1.5 Find user by display name (case-insensitive partial match)
...
### 2.1 All payments (most recent first)
...
### 2.2 Payment history for one user
...
### 2.3 Revenue summary (monthly, successful only)
...
### 2.4 Pending payments older than 1 hour (stale charges to investigate)
...
### 2.5 Failed payments (last 30 days, to identify issues)

```bash
node -e 'const db=require("better-sqlite3")("/data/app.db");const rows=db.prepare("SELECT id, user_id, method, failure_code, failure_message, substr(updated_at, 1, 16) AS failed_at FROM payments WHERE status IN (\"failed\",\"expired\") AND updated_at >= datetime(\"now\",\"-30 days\") ORDER BY updated_at DESC").all();console.table(rows);'
```

---

## 3. Food logs (engagement signal)

### 3.1 Today's logs across all users
...
### 3.2 Log volume by source (manual vs chat_ai vs photo)
...
### 3.3 Top 20 most-logged foods (popularity)
...
### 3.4 Heavy users (most logs in 30 days)

```bash
node -e 'const db=require("better-sqlite3")("/data/app.db");const rows=db.prepare("SELECT u.id, u.display_name AS name, u.plan, COUNT(f.id) AS logs_30d FROM users u JOIN food_logs f ON f.user_id = u.id WHERE f.logged_at >= datetime(\"now\",\"-30 days\") GROUP BY u.id ORDER BY logs_30d DESC LIMIT 20").all();console.table(rows);'
```

### 3.5 Logs for a specific user (today)

Replace `2` with user_id:
```bash
node -e 'const db=require("better-sqlite3")("/data/app.db");const rows=db.prepare("SELECT id, food_name_th AS food, kcal, protein_g AS p, carbs_g AS c, fat_g AS f, meal_type, source, substr(logged_at, 12, 5) AS time FROM food_logs WHERE user_id = 2 AND date = date(\"now\",\"localtime\") ORDER BY logged_at").all();console.table(rows);const t=db.prepare("SELECT SUM(kcal) AS kcal, SUM(protein_g) AS p, SUM(carbs_g) AS c, SUM(fat_g) AS f FROM food_logs WHERE user_id = 2 AND date = date(\"now\",\"localtime\")").get();console.log("Totals:", t);'
```

---

## 4. Chat / consultation (premium feature usage)

### 4.1 Recent chat messages

```bash
node -e 'const db=require("better-sqlite3")("/data/app.db");const rows=db.prepare("SELECT c.id, u.display_name AS user, c.role, substr(c.content, 1, 80) AS preview, c.refused, substr(c.created_at, 1, 16) AS at FROM chat_messages c LEFT JOIN users u ON u.id = c.user_id ORDER BY c.id DESC LIMIT 30").all();console.table(rows);'
```

### 4.2 Refused questions (out-of-scope monitoring)

Useful to refine SYSTEM_PROMPT — see what users actually ask that gets refused:
```bash
...
### 4.3 Chat quota usage today (per user)
...
### 5.1 Recent weight logs for a user (chart input)

Replace `2` with user_id:
```bash
node -e 'const db=require("better-sqlite3")("/data/app.db");const rows=db.prepare("SELECT id, weight_kg, date, source, substr(logged_at, 1, 16) AS at FROM weight_logs WHERE user_id = 2 ORDER BY logged_at DESC LIMIT 20").all();console.table(rows);'
```

### 5.2 Users actively tracking weight (logged in last 14 days)

```bash
node -e 'const db=require("better-sqlite3")("/data/app.db");const rows=db.prepare("SELECT u.id, u.display_name AS name, u.current_weight_kg AS current_kg, u.target_weight_kg AS target_kg, COUNT(w.id) AS logs_14d, MAX(w.logged_at) AS last_weigh_in FROM users u LEFT JOIN weight_logs w ON w.user_id = u.id AND w.logged_at >= datetime(\"now\",\"-14 days\") WHERE u.daily_calorie_goal IS NOT NULL GROUP BY u.id ORDER BY logs_14d DESC").all();console.table(rows);'
```
...
### 6.1 Acquisition funnel snapshot
...
These **change data**. Always run the corresponding SELECT first to confirm
the target row(s).
...
### 7.2 ⚠ Revoke premium (reset to free, expires_at NULL)
...
### 7.4 ⚠ Delete user + cascade (food_logs, weight_logs, chat_messages, subscriptions, payments)

`ON DELETE CASCADE` is set on related tables. Replace `99` with user_id:
```bash
...
### 8.4 Latest activity per user (helpful for inactivity sweep)

```bash
node -e 'const db=require("better-sqlite3")("/data/app.db");const rows=db.prepare("SELECT u.id, u.display_name AS name, u.plan, MAX(COALESCE(f.logged_at, w.logged_at, c.created_at, u.created_at)) AS last_seen FROM users u LEFT JOIN food_logs f ON f.user_id = u.id LEFT JOIN weight_logs w ON w.user_id = u.id LEFT JOIN chat_messages c ON c.user_id = u.id GROUP BY u.id ORDER BY last_seen DESC").all();console.table(rows);'
```
...
- **Truncate `line_user_id`** in display: `substr(line_user_id, 1, 10)` keeps the
  table readable; full ID still queryable via WHERE clause.
- **Date math**: SQLite's `datetime("now","-7 days")` is UTC. For Bangkok local,
  use `date("now","localtime")`. Most timestamps in this DB are stored UTC
  (default `datetime('now')`), except `food_logs.date` and `chat_messages.date`
  which are pre-computed in user TZ at insert.
- **`amount_satang / 100`** to convert to THB display.
- **PowerShell users**: this file's commands are for **Railway Console (bash)**.
  In PowerShell you'd need different quoting; not relevant since the DB lives
  on Railway, not locally.
- **REPL mode**: for interactive exploration, paste this once then type SQL
  freely:
  ```bash
  ...
  ```
  Then in REPL: `q("SELECT * FROM users LIMIT 5")`. Exit with `.exit`.
- **Add more queries here** as patterns emerge — this is meant to be a living
  runbook.
