...
const createSchema = z.object({
  ...
  kcal_low: z.number().min(0).max(5000).optional(),
  kcal_high: z.number().min(0).max(5000).optional(),
...
router.post('/', requireAuth, (req: Request, res: Response) => {
  ...
  try {
    const log = foodLogsRepository.create({
      ...
      kcal_low: parsed.data.kcal_low ?? parsed.data.kcal,
      kcal_high: parsed.data.kcal_high ?? parsed.data.kcal,
